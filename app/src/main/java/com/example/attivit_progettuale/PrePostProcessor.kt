...
import java.util.*
import com.example.attivit_progettuale.Result
...
object PrePostProcessor {
    ...
    private const val mOutputColumn = 13 // left, top, right, bottom, score and 80 class probability
    private const val mThreshold = 0.60f // score above which a detection is generated
    private const val mNmsLimit = 35
    ...
    fun nonMaxSuppression(
        ...
        while (i < boxes.size && !done) {
            if (active[i]) {
                val boxA = boxes[i]
                val rectA = Rect(boxA.left, boxA.top, boxA.right, boxA.bottom)
                selected.add(boxA)
                if (selected.size >= limit) break
                for (j in i + 1 until boxes.size) {
                    if (active[j]) {
                        val boxB = boxes[j]
                        val rectB = Rect(boxB.left, boxB.top, boxB.right, boxB.bottom)
                        if (IOU(rectA, rectB) > threshold) {
                            active[j] = false
                            numActive -= 1
                            if (numActive <= 0) {
                                done = true
